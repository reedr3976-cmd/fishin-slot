#!/usr/bin/env python3
"""ATR / volatility regime A vs B study (analysis only).

Does not change live scanner defaults or scoring thresholds.

Examples:
  python3 run_atr_regime_study.py --live
  python3 run_atr_regime_study.py --demo
"""
...
from config import (
    ATR_REGIME_REPORT_JSON,
    ATR_REGIME_REPORT_TXT,
...
from backtest.atr_regime import run_atr_regime_study
from backtest.metrics import MetricBag
from backtest.report_atr_regime import build_atr_regime_report


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="ATR regime A vs B OOS study")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("ATR regime study (analysis only; live scanner unchanged)...")
    ...
    print("Crypto off · original scoring · no live ATR filter")

    result = run_atr_regime_study(
        demo=demo,
        instruments=symbols,
        timeframes=timeframes,
        train_fraction=args.train_frac,
    )
    report = build_atr_regime_report(result)
    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    Path(ATR_REGIME_REPORT_TXT).write_text(report, encoding="utf-8")
    Path(ATR_REGIME_REPORT_JSON).write_text(
        json.dumps(_to_jsonable(result), indent=2), encoding="utf-8"
    )
    print(report)
    print(f"Saved: {ATR_REGIME_REPORT_TXT}")
    print(f"Saved: {ATR_REGIME_REPORT_JSON}")
    return 0
