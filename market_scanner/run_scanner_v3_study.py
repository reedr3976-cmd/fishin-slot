#!/usr/bin/env python3
"""Run Scanner V3 staged research study (analysis only — no live changes).

Usage:
  python run_scanner_v3_study.py
  python run_scanner_v3_study.py --demo
"""
...
from config import (
    OUTPUT_DIR,
    SCANNER_V3_REPORT_JSON,
    SCANNER_V3_REPORT_TXT,
    study_instruments,
)
from backtest.engine import load_series_map
from backtest.report_scanner_v3 import build_v3_payload, write_v3_reports


def main() -> int:
    p = argparse.ArgumentParser(description="Scanner V3 research study (4H)")
    ...
    print("Running V3 staged walk-forward study...", flush=True)
    payload = build_v3_payload(series_map)
    if errors:
        payload["load_errors"] = errors
    text = write_v3_reports(payload, SCANNER_V3_REPORT_TXT, SCANNER_V3_REPORT_JSON)
    print(text)
    print(f"Wrote {SCANNER_V3_REPORT_TXT}", flush=True)
    print(f"Wrote {SCANNER_V3_REPORT_JSON}", flush=True)
