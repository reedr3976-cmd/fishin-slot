...
Does not change the default live-equivalent fixed-hold backtest path unless
callers pass an ExitPolicy explicitly.
"""
...
class ExitPolicy:
    """How to exit after an ORIGINAL-rules entry.

    Modes:
      fixed — close at entry+horizon (baseline)
      stop  — ATR stop only; else timeout at horizon
      target — ATR take-profit only; else timeout at horizon
    """
...
def backtest_series_exits(
    ...
    *,
    ...
    """Walk-forward entries with ORIGINAL scoring; exit via ExitPolicy.

    When require_trending=True, only keep signals with sma_stack feature.
    Non-overlapping: advance by bars held (exit_idx - entry_idx).
    """
    rules = rules or ORIGINAL_RULES
    horizon = exit_policy.horizon_bars or FORWARD_BARS.get(series.timeframe)
    if horizon is None:
        return []
    ...
    n = len(series)
    last_start = n - 1 if end_idx_exclusive is None else min(n - 1, end_idx_exclusive)
    # Need room for at least 1 bar after entry
    last_start = min(last_start, n - 2)

    while i <= last_start:
        # Need full horizon available for fair timeout comparison
        if i + horizon >= n:
            break
        ...
        atr = float(opp.atr) if opp.atr is not None else None
        exit_idx, exit_px, reason, r_mult = simulate_exit(
            series, i, opp.direction, atr, exit_policy
        )
        entry = float(series.close[i])
        gross = _signed_move(opp.direction, entry, exit_px)
        net = gross - cost
        trades.append(
            TradeResult(
        ...
        # Non-overlapping: resume after exit
        i = max(exit_idx, i + 1)
    return trades
